...
import { FOCUS_VIS, getSepClass, getSepVars, LabelButton, PrBadge, resolveGlow } from "./shared";
...
function Frame({
	...
	const header = (
		<div
			{...headerProps}
			className={`${headerProps.className || ""} @container/hdr relative z-20 flex items-center gap-2 px-3 py-1.5 text-[11px] tracking-wider font-light shrink-0 select-none transition-[opacity,background,box-shadow] duration-300 ${sepClass}`}
			style={{
				...headerProps.style,
				height: 32,
		...
		>
			...
			<SessionHistoryTrigger
				className={`text-lg tracking-wider font-medium px-2 py-0.5 rounded-full cursor-pointer border-none hover:brightness-110 transition-all ${FOCUS_VIS}`}
				style={{
					background: `linear-gradient(135deg, ${c1}44, ${c2}44)`,
					color: theme.foreground,
					boxShadow: isFocused ? `0 0 6px ${c2}44` : "none",
